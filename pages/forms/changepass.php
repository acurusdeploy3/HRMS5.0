...
class LDAPAuth
{
	...
	var $ldap_dn = "ou=HRMS,ou=Test,ou=Acurus,dc=acurusldap,dc=com";
	...
	function ValidateUserGroup()
	{
		$filter ="(cn=*)";
		$justthese = array('cn');
		$result=ldap_list($this->conn, $this->ldap_dn, $filter, $justthese) or die("No search data found."); 
		$info = ldap_get_entries($this->conn, $result);
		for ($i=0; $i < $info["count"]; $i++) 
		{
				//echo $info[$i]["cn"][0] . '<br />';
				$search_filter	=	"(cn=".$info[$i]["cn"][0] .")";
				$result_info	= 	ldap_get_entries(
												$this->conn,
												ldap_search( 
															$this->conn,
