...
inner join employee_details ed on ei.employee_id=ed.employee_id where process_queue='HR_Manager_Process' and ei.is_active='Y' and SUBSTRING_INDEX(no_due_admin_allocated_to, '-', -1)=$userid 
and no_due_admin_status='Y'
order by date(date_of_leaving) asc");
$listquery=mysqli_query($db,"SELECT concat(First_name,' ',MI,' ',Last_Name,'-',employee_id) as accname FROM `employee_details` where job_role like 'Admin' and is_active='Y' and employee_id not in (3,'$userid')");
...
employee_comments,reporting_manager_comments,process_queue
FROM `employee_resignation_information` ei
inner join employee_details ed on ei.employee_id=ed.employee_id where  ei.is_active='N' and  SUBSTRING_INDEX(no_due_admin_allocated_to, '-', -1)=$userid order by date(date_of_leaving) asc");
		  ...
		  <h4 style="margin-left: 7px;font-weight: bold;color: green;">ACTIVE RESIGNATIONS</h4>
            <table id="resgnchange" class="table" style="font-size:14px;">
                <thead>
			      ...
			      <th>Status</th>
				  <th>View/Edit</th>
				  <?php if($userRole =='Admin' && mysqli_num_rows($listquery) >0 ){
				...
				echo "<td><a href='hradminnodueprocessingform.php?res_id=".$row['res_id_value']."'><i class='fa fa-check-circle-o' id='faicon'></i></a></td>";}
				if($userRole =='Admin' && mysqli_num_rows($listquery) >0 ){
		  ...
		  <h4 style="margin-left: 7px;font-weight: bold;color: tomato;">RESIGNATION HISTORY</h4>
            <table class="table" style="font-size:14px;width:100%" id="histchange">
                ...
                //  echo "<tr><td cols-span='4'> No Results Found </td></tr>";
                  ...
                  $i = 1;
                  while($row2 = mysqli_fetch_assoc($histquery)){
                    echo "<tr><td style='width:1%'>".$i.".</td>";
					echo "<td style='width:5%'>".$row2['employee_id']."</td>";
					echo "<td style='width:5%;display:none;'>".$row2['resignation_id']."</td>";
					echo "<td style='width:15%'>".$row2['Name']."</td>";
					echo "<td style='width:15%'>".$row2['Department']."</td>";
					echo "<td style='width:17%'>".$row2['queue']."</td></tr>";
					$i++;
				}
				}
                ?>
