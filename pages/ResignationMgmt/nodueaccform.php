...
inner join employee_details ed on ei.employee_id=ed.employee_id where process_queue='HR_Manager_Process' and ei.is_active='Y' and SUBSTRING_INDEX(no_due_acc_allocated_to, '-', -1)=$userid 
and no_due_acc_status='Y'
...
employee_comments,reporting_manager_comments,process_queue
FROM `employee_resignation_information` ei
inner join employee_details ed on ei.employee_id=ed.employee_id where  ei.is_active='N' and  SUBSTRING_INDEX(no_due_acc_allocated_to, '-', -1)=$userid order by date(date_of_leaving) asc");
		  ...
		  <h4 style="margin-left: 7px;font-weight: bold;color: green;">ACTIVE RESIGNATIONS</h4>
            <table id="resgnchange" class="table" style="font-size:14px;">
                <thead>
			      ...
			      <th>Status</th>
				  <th>View/Edit</th>
				  <?php if($userRole =='Accounts Manager'){
				...
				echo "<td><a href='accountsnodueprocessingform.php?res_id=".$row['res_id_value']."'><i class='fa fa-check-circle-o' id='faicon'></i></a></td>";}
