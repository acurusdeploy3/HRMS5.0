...
reporting_manager_comments,status,
date_format(date(date_of_submission_of_resignation),'%d-%b-%Y') as ds,
date_format(date(date_of_leaving),'%d-%b-%Y') as dl,hr_comments,hr_reason_update
FROM `employee_resignation_information`where employee_id=$id and is_active='Y' order by resignation_id desc limit 1 ");
...
$mailhod=$emhodrow['contact_email'];
	...
	//$db_handle = new DBController();
            ...
            // Build the message
            $mail->Subject = 'AHRMS - Exit - Resignation Acceptance Mail - '.$SenderNam.' - '.$id;
	...
	<tbody>
					...
					<tr style='background-color:#fff;'>
						...
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Hi ". $SenderNam .",</td>
					</tr>
					<tr style='background-color:#fff;'>
						
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Your Resignation has been accepted. As per Company policy, you shall stand relieved from your services at Acurus on ". $dateoflev ." subject to exit formalities. Feel free to call HR for further clarifications.</td>
					</tr>
					<tr style='background-color:#fff;'>
						
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:8px;'> <b>Note :</b> Your last working date is subjected to change based on the leave taken during the notice period.</td>
					</tr>
					<tr style='background-color:#fff;'>
