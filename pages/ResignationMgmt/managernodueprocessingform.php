...
if(isset($_POST['Submit'])){
 $query1 = mysqli_query($db,"SELECT * FROM `nodueformentries` where resignation_id=$resignation_id and department=
(select department from all_departments where department=(select department from employee_details
where employee_id=(select employee_id from employee_resignation_information where resignation_id=$resignation_id)))");
...
$managerQuery = mysqli_query($db,"SELECT * FROM `nodueformentries` where resignation_id=$resignation_id and department=
(select department from all_departments where department=(select department from employee_details
where employee_id=(select employee_id from employee_resignation_information where resignation_id=$resignation_id)))");
$depquery = mysqli_query($db,"select department from employee_details
where employee_id=(select employee_id from employee_resignation_information where resignation_id=$resignation_id)");
$deprow=mysqli_fetch_assoc($depquery);
$depname=$deprow['department'];
?>
<html>
<head>
  <meta charset="utf-8">
        ...
        <div class="col-md-11">
		  ...
		  <div class="border-class">
           <form class="form-horizontal" method="post" action="" enctype="multipart/form-data" >
              <div class="box-body">
				<label style="font-size: 20px;"><?php echo $depname ?></label>
				 <?php 
			  if(mysqli_num_rows($managerQuery) < 1){
                  echo "No Results Found";
                }else{ 
                  $i = 1;
				  echo "<div class='form-group'>";
                while($row = mysqli_fetch_assoc($managerQuery)){
				...
				<option value=".$row['status'].">Completed</option>
				<option value='No'>Not-Completed</option></select>";
				}
				else if($row['status']=='No')
				{
				echo "<select class='form-control' id=".$row['details_id']." name=".$row['details_id'].">
				<option value=".$row['status'].">Not-Completed</option>
