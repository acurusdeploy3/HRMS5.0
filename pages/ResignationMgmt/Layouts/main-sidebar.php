...
$getFirstLineLookup = mysqli_query($db,"select * from pms_manager_lookup where manager_id='$userid'");
	if(mysqli_num_rows($getFirstLineLookup)>0)
	{
		...
		$getishrresg = mysqli_query($db,"select * from application_configuration where module='RESIGNATION' and Parameter='HR_ID' and config_type='LIST_VIEW'	
and  locate(',$userid,',value)");	
	if(mysqli_num_rows($getishrresg)>0)	
	{	
		$getHrdetails='Y';	
	}
	...
	<li class="treeview">
		...
		<li class="treeview active">
          ...
          <?php
			...
			{
			?>
			<li><a href="noduehradmform.php"><i class="fa fa-circle-o"></i>Resignation of all employees</a></li>
			<?php } ?>
			<?php
			if($IsHod == 'Y')
			{
