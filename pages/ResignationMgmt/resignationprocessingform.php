...
$certQuery = mysqli_query($db,"SELECT  resignation_id,ei.employee_id,concat(First_name,' ',MI,' ',Last_Name) as Name,date_format(date(date_of_submission_of_resignation),'%d %b %Y') as ds,res_id_value,
...
employee_comments,reporting_manager_comments,hod_comments,hr_comments,process_queue,
...
where (process_queue='HOD_Process' or process_queue='HR_Manager_Hold' or process_queue='HR_Manager_Approved') and ei.employee_id <> $userid and h.location=ed.business_unit and ei.is_active='Y'
...
inner join employee_details ed on ei.employee_id=ed.employee_id
inner join all_hods h on ed.department=h.department
where ei.employee_id <> $userid and h.location=ed.business_unit and substring_index(ei.pending_queue_id,'-',1)<>$userid
and ei.is_active='N'
order by date(date_of_leaving) asc");
...
reason_for_resignation, status,   if((date_format(date(date_of_leaving),'%d %b %Y'))='01 Jan 0001','',(date_format(date(date_of_leaving),'%d %b %Y'))) as dl,
employee_comments,reporting_manager_comments,hod_comments,process_queue,exit_interview_status,
no_due_sysadmin_status,no_due_acc_status,no_due_manager_status,no_due_hr_status,res_id_value,no_due_admin_status,
if(Process_Queue='Employee_Process','Waiting for Manager Approval',
if(Process_Queue='Manager_Process','Waiting for HOD Approval',
if(process_queue='HOD_Process','Waiting in HR Manager Queue',
if(process_Queue='HR_Manager_Approved','Resignation Approved',
...
inner join employee_details ed on ei.employee_id=ed.employee_id 
inner join all_hods h on ed.department=h.department
where locate(',$userid-',pending_queue_id) and ei.process_queue='HR_Manager_Process' and ei.is_active='Y' and ei.employee_id <> $userid and h.location=ed.business_unit order by date(date_of_leaving) asc");
...
(select ifnull(sum(if(permission_type='Half Day',0.5,1)),0) from attendance_tracker where employee_id=ei.employee_id and date >= date(date_of_submission_of_resignation) and date <= date_of_leaving
and leave_type='LOP' and permission_type in ('','Half Day')),
(select ifnull(sum(if(permission_type='Half Day',0.5,1)),0) from attendance_tracker where employee_id=ei.employee_id and date <= date_of_leaving and date >=(select date(modified_date_and_time) from audit where module_name='Resignation Management'
and employee_id =ei.employee_id and module_id=resignation_id and description='date_of_leaving'
	and modified_by in ((select substring_index(substring_index(substring_index(value,',',3),',',-2),',',1)
  from application_configuration where config_type='LIST_VIEW' and module ='RESIGNATION'
	and parameter='HR_ID'),(select substring_index(substring_index(substring_index(value,',',3),',',-2),',',-1)
...
inner join employee_details ed on ei.employee_id=ed.employee_id
inner join all_hods h on ed.department=h.department
where ei.is_active='Y' and h.location=ed.business_unit
order by date_of_submission_of_resignation desc");
	...
	<section class="content">
      <div class="row">
        ...
        <div class="col-md-11">
		  ...
		  <h4 style="margin-left: 7px;font-weight: bold;color: green;">Exit Request(s) Awaiting Action</h4>
            <table id="resgnchange" class="table table-striped" style="font-size:14px;">
                ...
                //  echo "<tr><td cols-span='4'> No Results Found </td></tr>";
                  ...
                  $resid = $row['resignation_id'];
					...
					echo "<input type='hidden' class='queue' value='".$row['queue']."'></input>";
					echo "<input type='hidden' class='adl' value='".$row['adl']."'></input>";
					echo "<input type='hidden' class='dl' value='".$row['dl']."'></input>";
                  echo "<input type='hidden' class='ads' value='".$row['ds']."'></input>";
                  ...
                  echo "<input type='hidden' class='reporting_manager_comments' value='".$row['reporting_manager_comments']."'></input></td>";
                  echo "<input type='hidden' class='hod_comments' value='".$row['hod_comments']."'></input></td>";
                  echo "<input type='hidden' class='lop_count' value='".$row['lop_count']."'></input></td>";
					echo "<td style='width:25%'>".$row['queue']."</td>";
				if($row['process_queue'] == 'HOD_Process' || $row['process_queue'] == 'HR_Manager_Hold' ){
					echo "<td><a href='hrprocessingform.php?res_id=".$row['res_id_value']."'><i class='fa fa-pencil-square-o' id='faicon'></i></a></td>";
					$resid = $row['resignation_id'];
					$empidval = $row['employee_id'];
				}
				else if ($row['process_queue']=='HR_Manager_Approved')
				{
		   ...
		   <div id="myModal" class="modal">
					...
					<div class="modal-content">
						...
						<form id="statuschange" class="form-horizontal" method="POST" action="updatedateofleaving.php">
							...
							<div class="col-sm-5">
								<input type="text" class="form-control" id="dateleft" name="dateleft" style="background-color:white;" readonly required ></input>
							</div>
							</div>
							<div class='form-group'>
					...
					<div class="modal-content">
						...
						<table class="table">
							...
							<tr>
								...
								<td><input type="text" style="border-width:0px;border:none;width: -webkit-fill-available;" name="mngr" id="mngr" readonly></input></td>
							</tr>
							<tr>
		...
		<hr><div class="border-class">
		  <h4 style="margin-left: 7px;font-weight: bold;color: green;">Pending Exit Formalities</h4>
            <table id="exitchange" class="table table-striped" style="font-size:14px;">
                ...
                if(mysqli_num_rows($certQuery1) < 1){
                  ...
                  $i = 1;
                  while($row10 = mysqli_fetch_assoc($certQuery1)){
                    echo "<tr><td style='width:1%'>".$i.".</td>";
					echo "<td class='EmpId' style='width:6%'>".$row10['employee_id']."</td>";
					echo "<td style='width:15%'>".$row10['Name']."</td>";
					echo "<td style='width:13%'><a href='#' title='View Employee Data' id='empldata' data-toggle='modal' data-target='#datamodel'> <i class='fa fa-info-circle' aria-hidden='true'></i>";
					echo "<input type='hidden' class='Department' value='".$row10['Department']."'></input>";
					echo "<input type='hidden' class='employee_comments' value='".$row10['employee_comments']."'></input>";
					echo "<input type='hidden' class='Manager_Name' value='".$row10['Manager_Name']."'></input>";
					echo "<input type='hidden' class='Hod_Name' value='".$row10['Hod_Name']."'></input>";
					echo "<input type='hidden' class='reason_for_resignation' value='".$row10['reason_for_resignation']."'></input>";
					echo "<input type='hidden' class='queue' value='".$row10['queue']."'></input>";
					echo "<input type='hidden' class='adl' value='".$row10['adl']."'></input>";
                  echo "<input type='hidden' class='ads' value='".$row10['ds']."'></input>";
					echo "<input type='hidden' class='dl' value='".$row10['dl']."'></input>";
					echo "<input type='hidden' class='leave_count' value='".$row10['leave_count']."'></input></td>";
                  	echo "<input type='hidden' class='lop_count' value='".$row10['lop_count']."'></input></td>";
                  echo "<input type='hidden' class='reporting_manager_comments' value='".$row10['reporting_manager_comments']."'></input></td>";
                  echo "<input type='hidden' class='hod_comments' value='".$row10['hod_comments']."'></input></td>";
					
					echo "<td style='width:13%'>".$row10['queue']."</td>";
					echo "<td><a href='hrnodueprocessingform.php?res_id=".$row10['res_id_value']."'><i class='fa fa-file-text icon-stack-3x' id='faicon' title='No Due Process'></i><i class='fa fa-pencil icon-stack-1x' style='font-size: 18px ! important;padding-left: 1%;' id='faicon' title='No Due Process'></i></a></td>";
					if($row10['no_due_manager_status']=='C' && $row10['no_due_sysadmin_status']=='C' && $row10['no_due_acc_status']=='C' && $row10['no_due_hr_status']=='C' && $row10['exit_interview_status']=='F')
					{
					echo "<td><a href='updatecompletionstatus.php?resignation_id=".$row10['resignation_id']."' title='Process Completion' id='completion'><i class='fa fa-times-rectangle-o' style='color:red;font-size:17px;' aria-hidden='true'></i></a></td>";
					}
					else
					{
					echo "<td></td>";
					...
					if($row10['exit_interview_status']=='C' || $row10['exit_interview_status']=='F')
					{
					...
					if($row10['no_due_manager_status']=='C')
					{
					...
					if($row10['no_due_sysadmin_status']=='C')
					{
					...
					if($row10['no_due_admin_status']=='C')
                    {
...
<hr>
 ...
 <?php
		  ...
		  <div class="border-class" >
           	 <h4 style="margin-left: 7px;font-weight: bold;color: green;">Active Exit Request(s)</h4>
          <table id="listchange" class="table table-striped" style="font-size:14px;">
                ...
                if(mysqli_num_rows($listqry) < 1){
                  ...
                  while($row11 = mysqli_fetch_assoc($listqry)){
					...
					echo "<input type='hidden' class='queue' value='".$row11['queue']."'></input>";
                  echo "<input type='hidden' class='ads' value='".$row11['ds']."'></input>";
					echo "<input type='hidden' class='adl' value='".$row11['adl']."'></input>";
					echo "<input type='hidden' class='dl' value='".$row11['dl']."'></input>";
                  echo "<input type='hidden' class='reporting_manager_comments' value='".$row11['reporting_manager_comments']."'></input></td>";
                  echo "<input type='hidden' class='hod_comments' value='".$row11['hod_comments']."'></input></td>";
                  ...
                  echo "<input type='hidden' class='lop_count' value='".$row11['lop_count']."'></input></td>";
					...
					echo "<td style='width:20%'>".$row11['queue']."</td></tr>";
                    $i++;
                  }
				}
                ?>
...
<hr>
           <?php } ?>
				   ...
				   <div class="box box-info" style="width:110%;">
		  <div class="border-class">
		  <h4 style="margin-left: 7px;font-weight: bold;color: green;">History of Exit Request(s)</h4>
             <table id="histchange" class="table table-striped" style="font-size:14px;">
             ...
             <tbody>
                ...
                if(mysqli_num_rows($histquery) < 1){
                 // echo "<tr><td cols-span='4'> No Results Found </td></tr>";
                  ...
                  while($row15 = mysqli_fetch_assoc($histquery)){
                    echo "<tr><td style='width:1%'>".$i.".</td>";
					echo "<td class='EmpId' style='width:5%'>".$row15['employee_id']."</td>";
					...
					echo "<input type='hidden' class='queue' value='".$row15['queue']."'></input>";
					echo "<input type='hidden' class='adl' value='".$row15['adl']."'></input>";
                  echo "<input type='hidden' class='ads' value='".$row15['ds']."'></input>";
					echo "<input type='hidden' class='dl' value='".$row15['dl']."'></input>";
                  echo "<input type='hidden' class='reporting_manager_comments' value='".$row15['reporting_manager_comments']."'></input></td>";
                  ...
                  echo "<input type='hidden' class='lop_count' value='".$row15['lop_count']."'></input></td>";
                    ...
                    echo "<td style='width:15%'>".$row15['dc']."</td>";
                    }
					else if($row15['status']=='Process_Completed' || $row15['status']=='Process Resignation'){
						
                    echo "<td style='width:15%'>".$row15['dl']."</td>";}
					if ($row15['queue']=='Exit Process Completed'){
                  echo "<td style='width:10%'><a href='HistoryPdf/nodueform.php?res_id=".$row15['res_id_value']."'><i name= 'Generate' id='icon' class='fa fa-download' aria-hidden='true'  id='generate'></i></a></td>";
			  }
			  else 
			  {
			  ...
			  if ($row15['queue']=='Exit Process Completed'){
					echo "<td style='width:10%'><a href='HistoryPdf/exitinterviewhrform.php?res_id=".$row15['res_id_value']." '><i name= 'Generate' id='icon' class='fa fa-download' aria-hidden='true' id='generate'></i></a></td>";
			  }
			   else 
			  {
				  echo "<td style='width:10%'></td>";
...
<script>
  ...
  reporting_manager_comments = $(this).find('.reporting_manager_comments').val();
  ...
  $('#mngr').val(reporting_manager_comments);
  $('#hod').val(hod_comments);
  $('#empcomments').prop('title',employee_comments );
  $('#hod').prop('title',hod_comments );
  $('#mngr').prop('title',reporting_manager_comments );
