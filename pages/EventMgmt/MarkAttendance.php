...
$getAttendees = mysqli_query($db,"select a.employee_id,concat(b.first_name,' ',b.last_name,' ',mi) as Name,business_unit,department,has_attended,is_memento_received,is_memento_eligible from
	...
	<form id="resForm" method="post">
      <div class="row">
		...
		<div class="box-header">
			<br>
			  <br>
               <h4 class="box-title"><strong>Attendance Registration & Memento Acceptance of Employee(s)</strong></h4>
			  <br>
		...
		include("config.php");
				...
				$count = mysqli_num_rows($getAttendees);
			?>
   </form>        
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
				...
				<?php
				if($count > 0){
				while ($Employees = mysqli_fetch_assoc($getAttendees))
					{
				?>
                <tr>
				 ...
				 <td>
					<?php
					if($Employees['has_attended']=='')
					{
					?>
						<a href="MarkasPresent.php?empId=<?php echo $Employees['employee_id'] ?>&EventID=<?php echo $EventID ?>" class="submit" ><img alt='User' src='../../dist/img/Tick_Mark_Dark-512.png' width='18px' height='20px' /></a>
					<?php
					}
					else
					{
						if($Employees['has_attended']=='Y')
						{
					?>
				 ...
				 <?php  
				if($Employees['is_memento_eligible']=='Y')
					{
				 ?>
                	<td>
					<?php
					if($Employees['is_memento_received']=='')
					{
					?>
						<a href="MarkasReceived.php?empId=<?php echo $Employees['employee_id'] ?>&EventID=<?php echo $EventID ?>" class="submit" ><img alt='User' src='../../dist/img/Tick_Mark_Dark-512.png' width='18px' height='20px' /></a>
					<?php
					}
					else
					{
						if($Employees['is_memento_received']=='Y')
						{
					?>
                ...
                <?php
					...
					{
					?>	
					<td> <span class="badge bg-blue">Not Required</span></td>		
			<?php
					}
					?>
                 </tr>
