...
left join resource_management_Table c on a.employee_id=c.employee_id where a.training_id=$id and a.is_active='Y' and c.is_active='Y'");
      ...
      <div class="row">
        <div class="col-xs-12">
		 <table>
			  ...
			  <tr>
			  <td>
                    <button onclick="window.location.href='<?php echo $backbtn;?>'" type="button" class="btn btn-block btn-primary btn-flat">Back</button>
                  </td>
	...
	<?php
	include("config2.php");
	$chkSession = mysqli_query($db,"select * from training_sessions where has_started='Y' and training_id='$id' and is_active='Y'");
if($isTrainer == 'Y')
{
	if(mysqli_num_rows($chkSession)==0)
					{
?>
          <a href="#" target="_blank" data-toggle="modal" data-target="#modal-default-Trainee" class="btn btn-success pull-right">Add Trainee</a>
...
?>
        ...
        <div class="col-xs-12">
            ...
            <div class="box-header">
              <h3 class="box-title">Participants for Active Cycle</h3>
            ...
            <!-- /.box-header -->
             <div class="box-body no-padding">
              <table class="table table-striped">
				...
				<?php
				if(mysqli_num_rows($getparticipants)>0)
				{
					$i=1;
					while($Part = mysqli_fetch_assoc($getparticipants))
					{

				?>
                <tr>
                  <td><?php echo $i ?></td>
                  <td><?php echo $Part['employee_id'] ?></td>
				  ...
				  if($isTrainer == 'Y')
					{
				  ?>
				  <td><a href="RemoveTrainee.php?id=<?php echo $Part['participant_id'] ?>&Emplid=<?php echo $Part['employee_id']; ?>" OnClick="return confirm('Sure to Remove Participant?');"><img alt='User' src='../../dist/img/remove-icon-png-8.png' title="Remove Employee from Training" width='18px' height='18px' /></a></td>
				<?php
					}
					else
					{
...
session_duration,mode_of_training,if(is_completed='Y','Completed','Yet to Start') as status
 from training_sessions a left join employee_details b on a.trainer=b.employee_id where training_id='$id' and a.is_active='Y' order by start_date desc");
	  ...
	  <br>
        ...
        <div class="box-body">
		  ...
		  <div class="col-md-6">
			  ...
			  <?php
			  include("config.php");
			  $ExisTrainees  = mysql_query("select group_concat('''',employee_id,'''') as employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and Is_active='Y';");
...
on a.employee_id=b.employee_id where a.employee_id not in (select employee_id from all_trainers )");
			  $recaud = mysql_query("select distinct(department) as department from resource_management_table where department!='' and employee_id not in (select employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc' and Is_active='Y') and employee_id not in (select trainer from training_sessions where training_id='$id' and is_active='Y');");
			  $recaudRole = mysql_query("select distinct(designation) as designation from resource_management_table where department!='' and employee_id not in (select employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc' and Is_active='Y') and employee_id not in (select trainer from training_sessions where training_id='$id' and is_active='Y'); ");
...
on a.employee_id=b.employee_id where a.employee_id not in (select employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc' and Is_active='Y')  and a.employee_id not in (select trainer from training_sessions where training_id='$id' and is_active='Y') and a.is_active='Y';");
$samquery = "select a.employee_id,concat(First_Name,' ',Mi,' ',Last_Name,' : ',a.employee_id) as Name from resource_management_Table a left join employee_details b
on a.employee_id=b.employee_id where a.employee_id not in ($TIDs) and a.employee_id!='$userid' and a.is_active='Y';";
