...
{
?>
	  <a href="#" target="_blank" data-toggle="modal" data-target="#modal-default-Session" class="btn btn-success pull-right">Add Session</a>
	<?php
}
?>
        ...
        <div class="col-xs-12">
            ...
            <div class="box-header">
              <h3 class="box-title">Schedule for Active Cycle
		</h3>
            ...
            <div class="box-body no-padding">
              <table name="ActProj" id="ActProj" class="table table-striped">
                ...
                <tr>
                  ...
                  <td class="name"><?php echo $SessionRow['Name'];  ?></td>
...
on a.employee_id=b.employee_id where a.employee_id not in (select employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc' and Is_active='Y')");
...
on a.employee_id=b.employee_id where a.employee_id not in (select employee_id from all_trainers) and a.employee_id not in (select employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc' and Is_active='Y') and a.is_active='Y'");
			  $recaud = mysql_query("select distinct(department) as department from resource_management_table where department!='' and employee_id not in (select employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc' and Is_active='Y');");
			  $recaudRole = mysql_query("select distinct(designation) as designation from resource_management_table where department!='' and employee_id not in (select employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc' and Is_active='Y');");
...
on a.employee_id=b.employee_id where a.employee_id not in (select employee_id from all_trainers ) and a.employee_id not in (select employee_id from training_participants where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc' and Is_active='Y') and a.is_active='Y'");
            ...
            <div class="col-md-6">
               <div class="form-group">
                 <label>Topic</label>
                <input type="text" name="topicText" class="form-control" placeholder="Enter Topic of Interest" required="required" />
              </div>
			  <div class="form-group">
                <label>Start Time</label>
				    <div class="input-group">
                    <input type="text" id="startTime" name="startTime"  class="form-control timepicker" required="required" required>
			  ...
			  <div class="form-group">
                <label>Mode of Training</label>
                 ...
                 <select class="form-control select2" id="TrainingMode" name="TrainingMode" style="width: 100%;" required="required" required>
                 <option value="" selected disabled>Please Select from Below</option>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header" style="background-color:lightcoral">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">Training Management</h4>
              </div>
              <div class="modal-body">
                 <h5>Trainer Not Available for the Date and Time Provided. Please try a different Date.</h5>
              </div>
              <div class="modal-footer">
...
$('#AddtoSchedule').on('click', function() {
	var trainer = $('#TrainerSel').val();
	var dateofsession = $('#datepicker1').val();
	var timeofsession = $('#startTime').val();
	var topic = $('#topicText').val();
	var mode = $('#TrainingMode').val();
	if(trainer=='' || dateofsession=='' || timeofsession=='' || topic=='' || mode=='')
	{
...
function CheckTrainer()
{
	...
	var mode = $('#TrainingMode').val();
   $.ajax({
      url: 'TrainerCheck.php',
      type: 'post',
      data: {
      	'trainerid' : trainer,
		'sessiontime' : timeofsession,
      	'sessiondate' : dateofsession,

      },
      success: function(response){
      	if (response == 'taken')
		{
        alert("Trainer Unavailable for the Selected Date and Time. Please try a different date.");
		 ajaxindicatorstop();
		 return false;
      	}
		else {
