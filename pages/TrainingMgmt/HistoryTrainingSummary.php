...
<div class="wrapper">
 ...
 <div class="content-wrapper">
	...
	<?php
	 ...
	 <?php
			  include("config2.php");
			  $getTraining = mysqli_query($db,"select a.training_module_id,b.training_desc,training_department,frequency,current_occurence,total_occurence_per_frequency,is_evaluation_required,Date_format(a.created_datE_and_time,'%d - %b - %Y') as created_datE_and_time,cycle_id from history_active_trainings a
left join all_training_modules b on a.training_module_id=b.training_module_id where training_id=$id and cycle_id=$CycleId");
...
from history_training_participants a
left join employee_details b on a.employee_id=b.employee_id
left join resource_management_Table c on a.employee_id=c.employee_id where a.training_id=$id and cycle_id=$CycleId and a.is_active='Y'");
      ...
      <div class="row">
        <div class="col-xs-12">
		 <table>
			  ...
			  <tr>
			  <td>
                    <button OnClick="window.location='TrainingSummary.php?id=<?php echo $id; ?>'" type="button" class="btn btn-block btn-primary btn-flat">Back</button>
                  </td>
...
session_duration,mode_of_training,if(is_completed='Y','Completed','Yet to Start') as status,has_started
 from history_training_sessions a left join employee_details b on a.trainer=b.employee_id where training_id='$id' and cycle_id=$CycleId and a.is_active='Y' order by start_date asc");
 
 $getper= mysqli_query($db,"select
round(
(select count(is_completed) from history_training_Sessions where is_completed!='N' and training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc')
  /
 (select count(*) from history_training_Sessions where training_id='$id' and cycle_id='$CyclId' and occurence_count='$CurOcc')
   *
   100
  ) as percent");
                ...
                <tr>
				 ...
				 if($SessionRow['status']=='Completed')
				  ...
				  <td> <span class="badge bg-green">Session Completed</td>
					  ...
					  {
						?>
						<td> <span class="badge bg-yellow">Scheduled</td>
					 <?php 
					  }
					 ?>
			   </tr>
...
<?php				
	 ...
	 <div class="row">
        <div class="col-xs-12">
            ...
            <div class="box-header">
              <h3 class="box-title"><strong>Training Attendee(s)</strong></h3>

              <div class="box-tools">
              </div>
			    <br>
			<?php
			$getAttendance = mysqli_query($db,"select distinct(session_id) as session_id from history_training_attendance where training_id='$id'  and is_active='Y' and cycle_id=$CycleId");	
			$getAttendance1 = mysqli_query($db,"select a.employee_id,concat(b.First_Name,' ',b.Last_Name,' ',b.MI) as Name from history_training_participants a left join employee_details b on a.employee_id=b.employee_id where training_id='$id' and a.is_active='Y' and cycle_id=$CycleId");	
			?>
				</div>
                ...
                <tr>
                  ...
                  <th>Employee Name</th>
					<?php
					$i=1;
					$cnt= mysqli_num_rows($getAttendance);
					while($i <= $cnt)
					{
					?>
					<th><?php  echo 'Session '.$i; ?></th>
					 <?php
						$i++;
					}
					?>
				...
				<?php
				while($GetAttendanceRow = mysqli_fetch_array($getAttendance1))
				{
				?>
				  <tr>
					...
					<?php
					$getAttendeeStatus = mysqli_query($db,"select has_attended from history_training_attendance where employee_id='".$GetAttendanceRow['employee_id']."' and training_id='$id' and is_active='Y' and cycle_id=$CycleId");
					$i=1;
					while($getAttendeeStatusRow = mysqli_fetch_array($getAttendeeStatus))
					{
					?>
					
					<?php
						if($getAttendeeStatusRow['has_attended']=='Y')
						{
						?>
						<td><span class="badge bg-green">Attended</td>
						<?php
						}
						elseif($getAttendeeStatusRow['has_attended']=='N')
						{
						?>
	  ...
	  ?>
	   <div class="row">
        <div class="col-xs-12">
            ...
            <div class="box-header">
              <h3 class="box-title"><strong>Training Evaluation</strong></h3>

              <div class="box-tools">
              </div>
			    <br>
			<?php
			$getEvaluationCnt= mysqli_query($db,"select distinct(evaluation_count) from history_training_evaluation where training_id='$id' and is_active='Y' and cycle_id=$CycleId");
			$getAttendance1 = mysqli_query($db,"select a.employee_id,concat(b.First_Name,' ',b.Last_Name,' ',b.MI) as Name from history_training_participants a left join employee_details b on a.employee_id=b.employee_id where training_id='$id' and a.is_active='Y' and cycle_id=$CycleId");	
			?>
				</div>
                ...
                <tr>
                  ...
                  <th>Employee Name</th>
					<?php
					$i=1;
					$cnt1= mysqli_num_rows($getEvaluationCnt);
					while($i <= $cnt1)
					{
					?>
					<th><?php  echo 'Assessment '.$i; ?></th>
					 <?php
						$i++;
					}
					?>
				...
				?>
				  <tr>
					...
					<td><?php echo $GetAttendanceRow['Name']; ?></td>
					<?php
					$getAttendeeStatus = mysqli_query($db,"select if(a.marks_obtained='Did Not Attend','Did Not Attend',round((a.marks_obtained/a.max_score)*100)) as percentage from history_training_evaluation a where employee_id='".$GetAttendanceRow['employee_id']."' and training_id='$id' and is_active='Y' and cycle_id=$CycleId");
					$i=1;
					while($getAttendeeStatusRow = mysqli_fetch_array($getAttendeeStatus))
					{
					if($getAttendeeStatusRow['percentage']!='Did Not Attend')
						{
					?>


						<td><?php echo $getAttendeeStatusRow['percentage'].' %'; ?></td>
						<?php
						}
						else
						{
						?>
						<td><span class="badge bg-red"><?php echo $getAttendeeStatusRow['percentage']; ?></td>
