...
<?php
...
else
	{
	  ...
	  session_start();
	  $usergrp = $_SESSION['login_user_group'];
	  if($usergrp=='HR' || $usergrp=='HR Manager' || $isTrainer=='Y')
	  {
	  ?>
	 <div class="row">
        <div class="col-xs-12">
            ...
            <div class="box-header">
              ...
              <div class="box-tools">
			    ...
			    <br>
			<?php
			$getAttendance = mysqli_query($db,"select distinct(session_id) as session_id from training_attendance where training_id='$id' and is_active='Y'");
			$getAttendance1 = mysqli_query($db,"select a.employee_id,concat(b.First_Name,' ',b.Last_Name,' ',b.MI) as Name from training_participants a left join employee_details b on a.employee_id=b.employee_id where training_id='$id' and a.is_active='Y'");
			?>
				</div>
				...
				?>
				  <tr>
					...
					<?php
					$getAttendeeStatus = mysqli_query($db,"select has_attended from training_attendance where employee_id='".$GetAttendanceRow['employee_id']."' and training_id='$id' and is_active='Y'");
					$i=1;
	  ...
	  ?>
	   <div class="row">
        <div class="col-xs-12">
            ...
            <div class="box-header">
              <h3 class="box-title"><strong>Your Attendance</strong></h3>
...
left join training_sessions b on a.session_id=b.session_id
 where employee_id=$userid and a.training_id='$id' and a.is_active='Y' and b.is_active='Y'");
			$GetTeamAttendance = mysqli_query($db,"select * from employee_details where employee_id in
			(select employee_id from training_participants where training_id='$id') and reporting_manager_id=$userid");
			?>
				</div>
				...
				<?php
					$i=1;
				while($GetAttendanceRow = mysqli_fetch_array($getAttendance))
				{
        ...
        <div class="col-xs-12">
            ...
            <div class="box-header">
              ...
              <div class="box-tools">
			    ...
			    <br>
			<?php
			$getEvaluationCnt= mysqli_query($db,"select distinct(evaluation_count) from training_evaluation where training_id='$id' and is_active='Y'");
			$getAttendance1 = mysqli_query($db,"select a.employee_id,concat(b.First_Name,' ',b.Last_Name,' ',b.MI) as Name from training_participants a left join employee_details b on a.employee_id=b.employee_id where training_id='$id' and a.is_active='Y'");
			?>
				</div>
				...
				?>
				  <tr>
					...
					<td><?php echo $GetAttendanceRow['Name']; ?></td>
					<?php
					$getAttendeeStatus = mysqli_query($db,"select if(a.marks_obtained='Did Not Attend','Did Not Attend',round((a.marks_obtained/a.max_score)*100)) as percentage from training_evaluation a where employee_id='".$GetAttendanceRow['employee_id']."' and training_id='$id' and is_active='Y'");
					$i=1;
					while($getAttendeeStatusRow = mysqli_fetch_array($getAttendeeStatus))
					{
...
?>
	  ...
	  ?>
        ...
        <div class="col-xs-12">
            ...
            <div class="box-header">
              <h3 class="box-title"><strong>Your Evaluation Scores</strong></h3>

              <div class="box-tools">
              </div>
			    <br>
			<?php
			$getEvaluation = mysqli_query($db,"SELECT if(marks_obtained='Did Not Attend','Did Not Attend',round((marks_obtained/max_score)*100)) as percentage,date_of_assessment,marks_obtained,evaluation_topic,status FROM `training_evaluation` where employee_id='$userid' and training_id='$id' and is_active='Y'");
			?>
				</div>
				...
				<?php
				while($GetEvaluationRow = mysqli_fetch_array($getEvaluation))
				{
					$i=1;
				?>
				  <tr>
  ...
  <?php
	  ...
	  <br>
	  <?php
	  $getIncompleteSessions = mysqli_query($db,"select * from training_Sessions where training_id='$id' and is_completed='N' and is_active='Y'");
	  ?>
...
<div class="row no-print">
 <div class="col-xs-12">
		...
		if($isTrainer=='Y' || $usergrp=='HR Manager')
		{
			  if(mysqli_num_rows($getIncompleteSessions)==0)
			  {
			  ?>
				<button OnClick="window.location='FinishTraining.php?id=<?php echo $id; ?>'" type="button" class="btn btn-success pull-right">Finish & Initiate Next Cycle</button>
