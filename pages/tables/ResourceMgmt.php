      ...
      <div class="row">
		...
		<div class="box">
		 <br>
			...
			$_SESSION['previous']='ResourceMgmt.php';
			$count=0;
			...
			$query2= mysql_query("select distinct(concat(band,' ',designation,' ',level)) as designation from resource_management_table where is_active='Y'");
			//$query2= mysql_query("select distinct(position_name) as position_name from all_positions where position_name!=''");
			$query3= mysql_query("SELECT employee_id,concat(first_name,' ',last_name,' ',MI,'- ',employee_id) as name FROM `employee_details` where employee_id not in ('',0,'$userid') and is_active='Y';");
         ...
         ?>
          ...
          //  $where[] = 'V.city_id = '.$cityid.' AND V.is_enabled = "1"';
            $where[] = "concat(band,' ',designation,' ',level) like '%$role%' and a.is_active='Y' and b.is_active='Y' and a.employee_id!='$userid'";
				}
			if($depat != '') {

				$depat = trim($depat);
            //$where[] = 'V.vid = '.$placeid;
            $where[] = "a.department like '%$depat%' and a.is_active='Y' and b.is_active='Y' and a.employee_id!='$userid'";
		...
		if($proj != '') {
            ...
            $where[] = "d.project_id like '%$proj%' and a.is_active='Y' and b.is_active='Y'";
        }

				if($empl != '') {
            $empl = trim($empl);
            $where[] = "a.employee_id ='$empl' and a.is_active='Y' and b.is_active='Y' and a.employee_id!='$userid'";
				}
           ...
           // $query = "SELECT * FROM visiting_places AS V JOIN tourist_city AS C ON C.city_id = V.city_id $sWhere ";
				$query = mysql_query("select distinct(a.employee_id) as employee_id,concat(first_name,' ',last_name,' ',MI) as name,concat(band,' ',designation,' ',level) as designation,a.department,TIMESTAMPDIFF(MONTH,b.date_joined,now()) as Months
					...
					$echoquery = "select distinct(a.employee_id) as employee_id,concat(first_name,' ',last_name,' ',MI) as name,concat(band,' ',designation,' ',level) as designation,a.department,TIMESTAMPDIFF(MONTH,a.Created_Date_and_time,now()) as Months
