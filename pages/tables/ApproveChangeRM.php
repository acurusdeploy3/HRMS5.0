...
$id = $_GET['id'];
$NewVal = $_GET['NewVal'];
$GetNewReport = mysql_query("select concat(First_Name,' ',Last_Name,' ',MI) as name from employee_Details where employee_id='$NewVal'");
$GetNewReportName = mysql_fetch_array($GetNewReport);
$NewReport = $GetNewReportName['name'];
...
$updateRes = mysql_query("update resource_management_Table set reporting_manager='$NewVal', is_active='N', effective_from=curdate(),created_date_and_time=now() where res_management_id=$insertId");
...
$recipients = 
	...
	<tbody>
					...
					<tr style='background-color:#fff;'>
						...
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Changed Reporting Head :  <strong>". 
$NewReport." : ".$NewVal."</strong></td>
					</tr>
...
$name=$_SESSION['login_user'];			
$getNewMgmrDetails = mysql_query("select employee_id,concat(first_name,' ',last_name,' ',mi) as name,official_email from employee_Details where employee_id='$NewVal'");
...
$recipients = 
            ...
            // Build the message
            $mail->Subject = 'Reporting Change : Request Approved';
            //$mail->msgHTML(file_get_contents('contents.html'), dirname(__FILE__));
			$mail->msgHTML("<html><body>
	<div>
			<img src='acurus-logo.png' align='right' alt='logo' style='width:184px; margin:0 auto; align:right; display:block;'>
	...
	<tbody>
					...
					<tr style='background-color:#fff;'>
						...
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Reporting Change request raised by ".$RaiserName." has been approved. The following employee will be your direct report effective today.</td>
					</tr>
					
					<tr style='background-color:#fff;'>
						
		<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Employee Name : <strong>".$EmpName.' : '.$EmpID."</strong></td>
					</tr>										
					<tr style='background-color:#fff;'>
