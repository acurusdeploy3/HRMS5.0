...
$month= date("F");
$idval= $_GET['idval'];
$querymylst=mysqli_query($db,"
SELECT c.employee_id,concat(e.First_Name,' ',e.MI,' ',e.Last_Name) as Name,e.employee_designation as designation,t.department,c.date_joined,
c.cos_master_id,
Date_Of_Completion_of_Probation,concat(ed.First_Name,' ',ed.MI,' ',ed.Last_Name) as Manager_Name,
...
l.Process,if(category='' or category is null,'N','Y') as cnt
FROM `cos_master` c
...
left join cos_pip_summary cp on c.cos_master_id=cp.cos_master_id and cp.is_active='Y'
...
$AimsDet = mysqli_query($db1,"SELECT EmpId FROM employee_list where process in ('CHARGE POSTING', 'PAYMENT POSTING','CODING') and TL<>'TL' and empid='$EmpID'");
...
<div class="wrapper">
 ...
 <div class="content-wrapper">
   <section class="content-header">
	...
	<form class="form-horizontal" id="cosform" method="post" action="" onsubmit="CheckRequired(event);" enctype="multipart/form-data" autocomplete="off">
		<div class="border-class">
		    <div id="myCarousel" class="carousel slide" data-interval="false" data-ride="carousel">
				<div class="carousel-inner">
					<div class="item active" id="divisionmain">	
						<div class="form-group">
							...
							<div class="col-sm-5">
								...
								<input type="hidden" class="form-control" id="emphod" name="emphod" value="<?php echo $useridrow1['hod_id']; ?>" autocomplete="off" readonly disabled>
                            <input type="hidden" class="form-control" id="emphr" name="emphr" value="<?php echo $hrval; ?>" autocomplete="off" readonly disabled>
							</div>				  
						</div>
						<div class="form-group">
					...
					<div class="item"  id="divisionsub" style="height:70%">
						...
						<tbody>
							<tr style="font-weight: bold;text-align: -webkit-center;color: black ! important;">
								...
								<td style='width:5%'><a href="#" id="myBtn1" title="Click to Add Strength" data-toggle="modal" data-target="#modal-default-Education"><i class="fa fa-fw fa-plus" style="color: forestgreen;"></i></a></td>
								<div id="myModal1" class="modal" style="height:1000%">
                                 ...
                                 <span class="error">*  </span> <span> All fields are mandatory</span> <br>
						...
						<table class="table" id="strtable" style="font-size:14px;width:90%;margin: 0 auto;    table-layout: fixed;">
							<tbody>
								<tr id="rows" style="text-align: -webkit-center;">
									...
									<td style='width:5%'>
									<?php echo "<a href='DeletePIPValues.php?idval=".$idval."&pipval=".$Result1['COS_PIP_SUMMARY_ID']."&scnval=Manager'><i class='fa fa-trash' style='color: tomato;'></i></a>";?>
									</td>
								</tr>
							</tbody>
						</table>	
						...
						<div class="form-group">
							...
							<?php if($useridrow1['cnt']=='Y'){ ?>
								<input type= "submit" name= "btnSubmit" class="btn btn-info" value= "Confirm PIP" id="savefields" > </input>
							<?php  } else {?>
								<input type= "submit" name= "btnSubmit" class="btn btn-info" value= "Confirm PIP" id="savefields" disabled="disabled" > </input>
							<?php  } ?>
							</div>
						</div>
					</div>
...
function CheckRequired(event) {
		if (!confirm('Are you sure you want move the employee to Process Improvement Plan?')){
  that.preventDefault();
        ajaxindicatorstart("Processing..Please Wait..");}
  else{
	ajaxindicatorstart("Processing..Please Wait..");
	   var process= 'HOD_PIP_Approved';
       var EmpIdvalue = $('#empid').val();
	   var AllIdvalue = $('#emphr').val();
...
<script>
function getval(){
if( document.getElementById('category').value=='' || document.getElementById('expdev').value=='')
{
