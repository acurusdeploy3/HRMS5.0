...
(select employee_id from all_hods a where a.department=t.department and a.location=e.business_unit) as hod_id,l.Process
FROM `cos_master` c 
		    ...
		    <div id="myCarousel" class="carousel slide" data-interval="false" data-ride="carousel">
				<div class="carousel-inner">
					...
					<div class="item">
						...
						<?php if($useridrow1['department'] == 'RCM')
...
round((sum(a.Actual_Prod)/sum(a.Target_prod))*100,2) as Productivity,
round((1-(sum(Errors_Rec)/sum(Actual_Prod)))*100,2) as Quality from (select d.*,((target*Percentage)/100) as `% of Target`,((Actual_Prod-Errors_Rec)/Target_prod) as Eff_Prod,(Actual_Prod/Target_prod) as Prod,
...
where substring_index(user_name,'_',-1)='$EmpID' and date(dop) between '$RFD' and '$RTD') as b) as c
		...
		$myval=mysqli_num_rows($CallersDet);
			...
			?>
						...
						<div class="form-group">
							<label for="inputName" class="col-sm-5 control-label">Productivity % (Please justify in case of % less than SLA)<span class="error">*  </span> </label>
							<div class="col-sm-5">
								<input type="text" class="form-control" onkeypress="return isNumberKey(event)" id="empprod" maxlength="5" name="empprod" autocomplete="off" required>
							</div>				  
						</div>
						<div class="form-group">
							<label for="inputName" class="col-sm-5 control-label">Quality % (Please justify in case of % less than SLA)<span class="error">*  </span> </label>
							<div class="col-sm-5">
								<input type="text" class="form-control" id="empqual" onkeypress="return isNumberKey(event)" name="empqual" maxlength="5" autocomplete="off" required>
							</div>				  
						</div>
						<?php } else {}?>
						...
						<div class="form-group">
							<div class="col-sm-6" style='text-align:right;'>
								<input type= "submit" name= "btnSubmit" class="btn btn-info" value= "Save" id="savefields"  > </input>
							</div>
						</div>
					</div>
				</div>
