...
$empId=$_GET['id'];
$details = mysqli_query($db,"select employee_id,concat(Last_Name,',',First_name,'',MI) as name ,Date_of_Birth,Gender,Employee_Blood_Group,Employee_Personal_Email,country_code,Primary_Mobile_Number,Alternate_Mobile_number,emergency_contact_name,emergency_contact_relation,Vehicle_Name,Vehicle_reg,Marital_Status,Spouse_Name,Marriage_Date,About_acurus,Remarks,Employee_image,Position_Applied_For,
...
$qualdet=mysqli_query($db,"SELECT concat(course_name,' - ' ,specialization) as qualification, Institution, Location, From_year, To_Year, education_type, percentage_obtained, university FROM `employee_qualifications` where employee_id=$empId and course_name in('SSLC','Higher Secondary') and is_Active='Y'");
$qualcoldet=mysqli_query($db,"SELECT concat(course_name,' - ' ,specialization) as qualification, Institution, Location, From_year, To_Year, education_type, percentage_obtained, university FROM `employee_qualifications` where employee_id=$empId and course_name not in('SSLC','Higher Secondary') and is_Active='Y'");
$certdet=mysqli_query($db,"SELECT concat(course_name,' - ',certification_name) as qualification, description, course_offered_by, issued_date, expiry_date FROM `employee_certifications` where employee_id=$empId and is_Active='Y'");
$workdet=mysqli_query($db,"SELECT  company_name, designation, worked_from, worked_upto,
 ...
 FROM `employee_work_history` where employee_id=$empId and is_Active='Y'");
...
$docdet=mysqli_query($db,"SELECT  document_type, document_number, has_expiry, valid_from, valid_to, is_Active, document_path
		...
		<h1 class="special2">Acurus Employee Data Sheet</h1>
		  ...
		  <tbody>
            <table style="width:100%">
                <tr>
					<td style="width:19%" class="boxcolor special1"> Personal E Mail ID </td>
					<?php  echo "<td style='width:78%'>".$row['Employee_Personal_Email']."</td>"; ?>
					<td style="width:10%" class="boxcolor special1"> Date of Information </td>
					<?php  echo "<td style='width:10%'>".$row['Employee_Personal_Email']."</td>"; ?>
				</tr>
			</table>
			<table style="width:100%">	
				<tr>
			...
			<table style="width:100%">
				...
				<td style="width:15%" class="boxcolor"><b>% Of Marks</b></td>
                  ...
                  while($row4 = mysqli_fetch_assoc($certdet)){
                    ...
                    echo "<td style='width:10%'>".$row4['issued_date']."</td>";
					echo "<td style='width:10%'>".$row4['expiry_date']."</td>";
			...
			<table style="width:100%">
				...
				<?php
                  ...
                  while($row5 = mysqli_fetch_assoc($workdet)){
                    ...
                    echo "<td style='width:20%'>".$row5['worked_from']."</td>";
                    echo "<td style='width:10%'>".$row5['worked_upto']."</td>";
					echo "<td style='width:10%'>".$row5['work_duration_months']."</td>";
					echo "<td style='width:15%'>".$row5['reason_for_leaving']."</td>";
		...
		<h1 class="special2">Acurus Employee Data Sheet</h1>
		 ...
		 <div class="border-class">
		  ...
		  <table style="width:100%">
			...
			<?php
				  $n=1;
                  while($row7 = mysqli_fetch_assoc($docdet)){
					echo "<tr>" ; 
					echo "<td style='width:5%'>".$n."</td>";
                    echo "<td style='width:25%'>".$row7['document_number']."</td>";
                    echo "<td style='width:20%'></td>";
                    echo "<td style='width:20%'></td>";
                    echo "<td style='width:10%'>".$row7['valid_to']."</td>";
