...
(select employee_id from all_hods a where a.department=t.department and a.location=e.business_unit) as hod_id,Manager_ID,
date_format(c.date_joined,'%d %b %Y') as doj,
date_format(c.Date_Of_Completion_of_Probation,'%d %b %Y') as doc,
if(category='' or category is null,'N','Y') as cnt,
l.Process
FROM `cos_master` c 
				...
				<div class="carousel-inner">
					<div class="item active">	
						<div class="form-group">
							...
							<div class="col-sm-5">
								...
								<input type="hidden" class="form-control" id="empmngr" name="empmngr" value="<?php echo $useridrow1['Manager_ID']; ?>" autocomplete="off" readonly disabled>
							</div>				  
						</div>
						<div class="form-group">
					...
					<div class="item" style="height:70%">
							...
							<tr style="font-weight: bold;text-align: -webkit-center;color: black ! important;">
								...
								<div id="myModal1" class="modal">
									<div class="modal-content">
										...
										<input type="text"  class="form-control" id="category" name="category" placeholder="Enter Category" /><br>
						...
						<table class="table" id="strtable" style="font-size:14px;width:90%;margin: 0 auto;    table-layout: fixed;">
							<tbody>
								<tr id="rows" style="text-align: -webkit-center;">
									...
									<td style='width:5%'>
									<?php if($scnval=='N') { echo "<a href='DeletePIPValues.php?idval=".$idval."&pipval=".$Result1['COS_PIP_SUMMARY_ID']."&scnval=Approval'><i class='fa fa-trash' style='color: tomato;'></i></a>"; } else {}?>
									</td>
									<div id="myModal1" class="modal">
									<div class="modal-content">
										<span class="close12">&times;</span>
						...
						<div class="form-group">
							...
							<?php if($scnval=='Y') { ?>
							<input type= "button" name= "savePIPfields" class="btn btn-info" value= "Send PIP Corrections" id="savePIPfields" onclick="sendforcorrections();" > </input>
							<?php } ?>
                        <?php if($useridrow1['cnt']=='Y'){ ?>
								<input type= "submit" name= "btnSubmit" class="btn btn-info" value= "Approve PIP" id="savefields"> </input>
      <?php  } else {?>
      <input type= "submit" name= "btnSubmit" class="btn btn-info" value= "Approve PIP" id="savefields" disabled="disabled"> </input>
    <?php  } ?>
							</div>
						</div>
					</div>
...
<script>
function sendforcorrections() {
		if (!confirm('Are you sure you want to save the changes?')){
  that.preventDefault();}
  else{
	ajaxindicatorstart("Processing..Please Wait..");
	   var process= 'PIP_Corrections';
       var EmpIdvalue = $('#empid').val();
	   var AllIdvalue = $('#empmngr').val();
	   var CosIdValue = $('#empcosid').val();
	   var StrValue = $('#empstr').val();
	   var ImpValue = $('#empimp').val();
	   var FedValue = $('#empfeed').val();
