...
if(mysqli_num_rows($checkifMngr)>0)
{
?>
<?php
include("config.php");
...
<h4 class="box-title"><strong>Current & Future Employee Shifts</strong>
			 <?php
			 include("config.php");
			 $getEmpShifts = mysqli_query($att,"SELECT shift_code,start_date,end_date FROM `employee_shift` where employee_id=$employeeid and end_date >= curdate() order by start_Date asc;");
			 ?>
			  </h4>  
