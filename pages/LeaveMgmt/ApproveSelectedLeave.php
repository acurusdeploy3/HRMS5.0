...
$selectReIDS = $_POST['selectLeaves'];
$totalselected = 1;
$reqIdsMail='';
foreach($selectReIDS as $reqID)
{
		if($totalselected == count($selectReIDS))
        {
        		$reqIdsMail.=$reqID.'';
        }
		else
...
echo $reqIdsMail;
$getAllemails = mysqli_query($db,"select official_email, employee_id from employee_details where employee_id in ( select distinct employee_id from leave_request where req_id in (".$reqIdsMail."))");
echo "select official_email, employee_id from employee_details where employee_id in ( select distinct employee_id from leave_request where req_id in (".$reqIdsMail."))";
while($emailrows = mysqli_fetch_assoc($getAllemails))
{
$emailval = $emailrows['official_email'];
$getleaveDates = mysqli_query($db,"select leave_from,leave_to,leave_type from leave_request where employee_id='".$emailrows['employee_id']."' and req_id in (".$reqIdsMail.")");
$finalleaves='';
