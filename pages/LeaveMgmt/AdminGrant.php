...
$RepMgmrName='Attendance Support';
...
if ($LeaveType!='Permission' && $LeaveType!='Privilege & Sick' && $LeaveType!='Casual & Sick')
{
		mysqli_query($db,"insert into leave_request
(employee_id,allocated_to,leave_type,number_of_days,status,leave_for,created_date_and_time,created_by,reason,
leave_from,leave_to)
...
$leaveFrom." </strong></td>
	...
	$OtherStakeArr = mysqli_query($db,"select a.employee_id,receipent,official_email from additional_Receipents a join employee_details e on a.receipent=e.employee_id where a.employee_id=$EmpID union select employee_id,name,email from additional_stakeholders where employee_id=$EmpID;");	
	$NumRows = mysqli_num_rows($OtherStakeArr);
	if(mysqli_num_rows($OtherStakeArr)>0)
	{
            ...
            // Build the message
            $mail->Subject = 'Attendance Support : '.$EmpName.' : '.$EmpID;
            //$mail->msgHTML(file_get_contents('contents.html'), dirname(__FILE__));
			$mail->msgHTML("<html><body>
				<div>
					<img src='AHRMS_Logo3.png' align='right' alt='logo' style='width:220px;height:100px; margin:0 auto; align:right; display:block;'>
			...
			<table style=' width:100%; margin:0 auto; font-family:Open Sans, sans-serif; border-collapse:collapse;' >
				<tbody>
					...
					<tr style='background-color:#fff;'>
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Permission for the following employee has been granted through support login. Kindly revert us back if it is not raised by the reporting head. The details of the request are as follows. This is just for your kind information.</td>
					</tr>
					
					<tr style='background-color:#fff;'>
						
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Employee Name & ID : <strong>".$EmpName.' : '.$EmpID."</strong>.</td>
					</tr>
...
$leavefor."</strong></td>
					...
					<tr style='background-color:#fff;'>
						...
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Permission Required Date  : <strong>".date("d M Y", strtotime($leaveFrom))." </strong></td>
					</tr>
	...
	if(mysqli_num_rows($OtherStakeArr)>0)
	{
			...
			<table style=' width:100%; margin:0 auto; font-family:Open Sans, sans-serif; border-collapse:collapse;' >
				<tbody>
					...
					<tr style='background-color:#fff;'>
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>".$LeaveTypeAb." ( ".$LeaveType." )  for the following employee has been granted through support login. Kindly revert us back if it is not raised by the reporting head. The details of the request are as follows. This is just for your kind information.</td>
					</tr>
					...
					<tr style='background-color:#fff;'>
						...
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Number of Days: <strong>". $NumDays." Day(s)</strong></td>
					</tr>
					
					<tr style='background-color:#fff;'>
						
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Request For: <strong>". $leavefor."</strong></td>
					</tr>
					
					<tr style='background-color:#fff;'>
						
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Required From  : <strong>". date("d M Y", strtotime($leaveFrom))." </strong></td>
					</tr>
					
						<tr style='background-color:#fff;'>
						
						<td style='padding-left: 30px; padding-right: 30px; padding-bottom:15px; padding-top:15px;'>Required To  : <strong>".date("d M Y", strtotime($leaveTo)) ." </strong></td>
					</tr>
