  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="#">PMS</a></li><li class="active">Deliverables</li>
      </ol>
    </section>
      ...
      <div class="box">
        ...
        <div class="box-body">
          <div class="border-class">
            <table class="table">
              <tbody>
                ...
                foreach($performanceData as $value) {
                  echo "<tr><th colspan='8' class='pms_head'>".@$value['head'][0]."</th></tr>";
                  foreach($value['criteria'] as $key => $val){
                    ...
                    foreach ($value['data'][$key] as $k => $v) {
                      $scoresQuery = mysqli_query($db,"select performance_score_id, appraisee_comments, measure_score, manager_comments from performance_scores a,performance_measures b where a.measure_id = b.measure_id and employee_id = $appraiseeId and a.is_Active = 'Y' and a.measure_id = ".$v[0]);
                      if(mysqli_num_rows($scoresQuery) > 0){
                        ...
                        $managerComments = @$scoresData['manager_comments'];
                        $performanceScoreId = @$scoresData['performance_score_id'];
                      }else{
                    ...
                    echo "<td id='appraisee_comments_".$criteriaId."'>".$appraiseeComments."</td>";
                    echo "<td><textarea rows='1' class='form-control' id='manager_comments_".$criteriaId."'>".$managerComments."</textarea></td>";
                    echo "<td><a href='javascript:updateManagerDeliverablesPerformance(\"".$criteriaId."\",\"".$performanceScoreId."\",".$category_id.",".$appraiseeId.");'><i class='fa fa-save'></i></a></td></tr>";
                  }
                }
                ?>
              </tbody>
            ...
            <table class="table">
              <tbody>
                <tr><th colspan='6' class='pms_head'>OverAll Performance</th></tr>
                <tr id="overAllData">
                <?php
                $totalWeightage = 0;
                ...
                $percentage = number_format(($totalWeightage * 0.8), 2, '.', '');
                $satisfactory = getPercentage($totalWeightage);
                ?>
                </tr>
                <tr id="overAllPerformance">
                <?php
                echo "<th>Total Weightage</th><td>".$totalWeightage."</td><th>Weightage of Deliverables(80%)</th><td>".$percentage."</td><th>Assessment</th><td>".$satisfactory."</td>";
                ?>
                </tr>
              </tbody>
            </table>
          ...
          <a href="managerCulturePerformance.php?appraisee_id=<?php echo $appraiseeId; ?>" class="btn btn-info" role="button" style="float:right;margin-right:20px;">Next >></a>
        </div>
        <!-- /.box-body -->
